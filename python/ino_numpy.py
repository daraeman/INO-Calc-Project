...
numpyrand = np.random.randint(1,98, size=5001)
numpypos = 0
...
for i in range(100000):
    ...
    while (done > 0):
        ...
        #newnum = numpy.random.randint(0,97)

        if (numpypos > 5000) :
            numpyrand = np.random.randint(0,98, size=5001)
            numpypos = 0

        # if the number is new then mark it as seen
        if receipts[ numpyrand[numpypos] ] == 0:
            receipts[ numpyrand[numpypos] ] = 1
